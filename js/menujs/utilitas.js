...
// [ROUTING] Konfigurasi Nama File sesuai Penyesuaian Baru
window.CORE_UTILITY_MODULES = {
  inventaris: { title: "Modul Inventaris & UKS", url: "modules/inventaris/inventaris.html", type: "iframe", akses: "guest" },
  kalender: { title: "Modul Kalender & Agenda Tahunan", url: "modules/kalender/kalender.html", script: "modules/kalender/kalender.js", initFn: "initKalenderModule", type: "inline", akses: "guest" },
  kta: { title: "Modul Cetak Kartu Tanda Anggota (KTA)", url: "modules/kta/kta.html", type: "iframe", akses: "anggota" },
  materi: { title: "Pustaka Materi & Dokumen Belajar", url: "modules/materi/materi.html", script: "modules/materi/materi.js", initFn: "initMateriModule", type: "inline", akses: "guest" },
  pelaporan: { title: "Modul Pelaporan Administrasi Resmi", url: "modules/pelaporan/laporan.html", type: "iframe", akses: "anggota" }
};
...
window.loadDynamicUtilityLinks = async function() {
  ...
  try {
    ...
    (extApps || []).forEach(app => {
      ...
      const sanitize = window.sanitizeHTML || (x => x);
      const amanNama = sanitize(app.nama);
      ...
      const itemHtml = `
        <div class="util-app-card util-dynamic-item" onclick="window.open('${app.target_url}', '_blank')">
          ...
          <div class="util-app-icon-wrap" style="background-color: ${app.warna_bg || '#ecfdf5'}; color: ${app.warna_aksen || '#059669'};">
            <img src="${app.ikon_url}" alt="${amanNama}" class="tool-squircle-img" />
          </div>
          <div class="util-app-title">${amanNama}</div>
...
window.handleTambahLinkSubmit = async function(event) {
  ...
  try {
    ...
    if (fileInput.files && fileInput.files[0]) {
      ...
      const allowedTypes = ['image/jpeg', 'image/png', 'image/webp']; // Abaikan SVG demi keseragaman raster (aman)
      ...
      const compressedBlob = typeof window.compressProfileImage === "function"
        ...
        : file;
      const fileName = `app_icon_${crypto.randomUUID()}.jpg`; // PERBAIKAN UUID
