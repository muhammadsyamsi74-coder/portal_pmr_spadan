 ...
 * Diperbarui: Tabel berjejer vertikal, auto-hide jika data kosong,
 * pembatasan data non-aktif khusus Admin & Pengurus.
 * ==============================================================================
 */
...
/* Helper Evaluasi Wewenang Pengurus / Admin */
  ...
  const generateRows = (arr) => {
    return arr.map(item => {
      ...
      if (item.golongan_darah) {
        goldarText = `${item.golongan_darah} ${item.rhesus_darah ? (item.rhesus_darah === 'Positif' ? '(+)' : '(-)') : ''}`;
      }
      
      const avatarImg = item.foto_profil_url
        ? `<img src="${item.foto_profil_url}" style="width: 36px; height: 36px; border-radius: 50%; object-fit: cover; object-position: top; border: 1px solid #ddd; margin: 0 auto; display: block; cursor: zoom-in;" onclick="window.showEnlargedPhoto('${item.foto_profil_url}')" title="Lihat Foto" />`
      ...
      return `
        <tr>
          ...
          <td>
            <div style="font-weight: 700; color: #222;">${item.nama_lengkap} <span style="font-weight: 500; color: #666;">(${item.nama_panggilan || "-"})</span></div>
            <div style="font-size: 11px; color: #888; margin-top: 2px;">NISN: ${item.nisn || "-"} ${jkLabel}</div>
          </td>
          <td>
            <div style="font-weight: 700; color: #333;">${item.kelas}</div>
            <div style="font-size: 11px; color: #777; margin-top: 2px;">Th. Bergabung: ${item.tahun_bergabung || "-"}</div>
          </td>
          <td>
            <div style="font-weight: 700; color: #333;">${item.keterangan_jabatan || "-"}</div>
          ...
          <td style="text-align: center; white-space: nowrap;">
            ...
            ${(isAdmin && currentUserId !== item.id) ? `<button class="btn-icon-action" type="button" title="Hapus Anggota" onclick="window.deleteAnggota('${item.id}', '${item.nama_lengkap}')"><i data-lucide="trash-2" style="color: #c62828;"></i></button>` : ""}
          </td>
        </tr>
      `;
    }).join("");
...
window.renderStructureGrid = function(aktifList) {
  ...
  const renderCard = (u) => {
    const photoEl = u.foto_profil_url
      ? `<img src="${u.foto_profil_url}" class="org-card-photo" alt="${u.nama_panggilan || u.nama_lengkap}" onclick="window.showEnlargedPhoto('${u.foto_profil_url}')" title="Lihat Foto" />`
      : `<div class="org-card-avatar">${(u.nama_panggilan || u.nama_lengkap || "?").charAt(0).toUpperCase()}</div>`;

    return `
      <div class="org-card">
        ${photoEl}
        <div class="org-card-name">${u.nama_lengkap}</div>
        <div class="org-card-sub">Panggilan: ${u.nama_panggilan || "-"} • Kelas ${u.kelas || "-"}</div>
        <div class="org-card-role">${u.keterangan_jabatan || "Anggota"}</div>
        <div class="org-card-year">Th. Bergabung: ${u.tahun_bergabung || "-"}</div>
      </div>
    `;
  };
...
window.handleAnggotaFilter = function() {
  ...
  const filtered = window.rawAnggotaList.filter((item) => {
    ...
    // Sembunyikan anggota non-aktif jika bukan pengurus/admin
